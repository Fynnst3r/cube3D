...
static void	place_hands(t_game *g)
{
	...
	if (mlx_image_to_window(g->mlx, g->hands[0],
			x2, g->mlx->height - g->hands[0]->height) < 0)
		ft_error("Error\nImage didn't arrive at window", 8, g);
	if (mlx_image_to_window(g->mlx, g->hands[1],
			x1, g->mlx->height - g->hands[1]->height) < 0)
		ft_error("Error\nImage didn't arrive at window", 9, g);
	if (mlx_image_to_window(g->mlx, g->hands[2],
			x1, g->mlx->height - g->hands[2]->height) < 0)
		ft_error("Error\nImage didn't arrive at window", 10, g);
	if (mlx_image_to_window(g->mlx, g->hands[3],
			x1, g->mlx->height - g->hands[3]->height) < 0)
